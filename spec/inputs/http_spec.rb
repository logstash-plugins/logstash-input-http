...
describe LogStash::Inputs::Http do
  ...
  describe 'request handling' do
    ...
    before :each do
      subject.register
      Thread.new { subject.run(logstash_queue) }
      ok = false
      until ok
        begin
    ...
    describe 'handling overflowing requests with a 429' do
      let(:logstash_queue_size) { rand(1..10) }
      let(:max_pending_requests) { rand(1..5) }
      let(:threads) { rand(1..4) }
      ...
      context 'when sending more requests than queue slots' do
        it 'should block when the queue is full' do
          ...
          logstash_queue_size.times.each do
            response = client.post("http://127.0.0.1:#{port}", :body => '{}').call
            expect(response.code).to eq(200)
          end

          # these will block
          (threads + max_pending_requests).times.each do
            expect {
              client.post("http://127.0.0.1:#{port}", :body => '{}').call
    ...
    context 'when using a custom codec mapping' do
      subject {
        LogStash::Inputs::Http.new('port' => port,
  ...
  context 'with :ssl => true' do
    ...
    context 'with :ssl_certificate' do
      ...
      subject {
        LogStash::Inputs::Http.new('port' => port, 'ssl' => true,
