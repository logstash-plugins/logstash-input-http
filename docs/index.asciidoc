...
In addition to decoding the events, this input will add HTTP headers of the connection to each event.
When ECS compatibility is disabled, the headers are stored in the `headers` field, which had the potential to create confusion and schema conflicts downstream.
When ECS is enabled, we can ensure a pipeline still has access to this metadata throughout the event's lifecycle without polluting the top-level namespace.
...
| [headers] | [@metadata][http][header] | Always | Full headers metadata
| [headers][http_version] | [http][version] | Always | HTTP version
| [headers][http_user_agent] | [user_agent][original] | Always | browser user agent
...
** `v1`: structured header metadata added under `[@metadata][http][header]`
